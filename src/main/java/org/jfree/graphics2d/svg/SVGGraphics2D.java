...
public final class SVGGraphics2D extends Graphics2D {
    ...
    private void appendOptionalElementIDFromHint(StringBuilder sb) {
        ...
        String elementCustom = (String) this.hints.get(SVGHints.KEY_ELEMENT_CUSTOM);
        if (elementCustom != null) {
            this.hints.put(SVGHints.KEY_ELEMENT_CUSTOM, null); // clear it
            this.sb.append(elementCustom + " ");
        }
    }
    ...
    public void fill(Shape s) {
        if (s instanceof Rectangle2D) {
            ...
            this.sb.append(getClipPathRef());
            this.sb.append(">");
            this.sb.append("<path vector-effect=\"non-scaling-stroke\" ").append(getSVGPathData(path)).append("/>");
            this.sb.append("</g>");
        }  else {
    ...
    public String getSVGElement(String id) {
        return getSVGElement(id, -1, -1, null, null, null);
    }
     ...
     * 
     * @param id  the element id ({@code null} permitted).
     * @param lwidth  the width ({@code null} permitted)
     * @param lheight  the height ({@code null} permitted)
     ...
     * @since 3.2
     */
    public String getSVGElement(String id, @Nullable Integer lwidth, @Nullable Integer lheight,
        ...
        if(lwidth != null)
        {
            if (lwidth == -1)
            {
                svg.append("width=\"").append(this.width).append(unitStr).append("\" ");
            }
            else
            {
                svg.append("width=\"").append(lwidth).append(unitStr).append("\" ");
            }
        }
        if(lheight != null)
        {
            if (lheight == -1)
            {
                svg.append("height=\"").append(this.height).append(unitStr).append("\" ");
            }
            else
            {
                svg.append("height=\"").append(lheight).append(unitStr).append("\" ");
            }
        }
        if (viewBox != null) {
